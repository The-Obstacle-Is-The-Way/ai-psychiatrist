...
class ReferenceStore:
    ...
    def _load_embeddings(self) -> dict[int, list[tuple[str, list[float]]]]:
        ...
        # BUG-001: Check for secure JSON file first, fallback to unsafe pickle with warning
        json_path = self._embeddings_path.with_suffix(".json")
        raw_data: dict[str, Any] = {}

        if json_path.exists():
            logger.info("Loading reference embeddings from JSON", path=str(json_path))
            with json_path.open("r", encoding="utf-8") as f:
                raw_data = json.load(f)
        elif self._embeddings_path.exists():
            logger.warning(
                "Loading reference embeddings from PICKLE (unsafe if untrusted).",
                path=str(self._embeddings_path),
            )
            # SECURITY: This pickle file is a trusted internal artifact generated by our
            # embedding pipeline. However, we are migrating to JSON/SafeTensors.
            # Only use this fallback for legacy data.
            warnings.warn(
                "Loading embeddings from pickle is deprecated and unsafe. Convert to JSON.",
                DeprecationWarning,
                stacklevel=2,
            )
            ...
            logger.warning(
                "Embeddings file not found (checked .json and .pkl)",
                path=str(self._embeddings_path),
            )
            self._embeddings = {}
            return self._embeddings
