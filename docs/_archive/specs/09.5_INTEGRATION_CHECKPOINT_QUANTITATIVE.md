...
### Real Ollama Gate (Opt‑In, Required Before Claiming Readiness)

Mocked integration tests prove architecture, but they do **not** validate real
LLM behavior. At this checkpoint we require at least one real Ollama run per
agent and a full `/full_pipeline` run (structural assertions only).

**Opt‑in Commands**:
```bash
# Requires a live Ollama daemon + models present (see docs/archive/bugs/BUG-018_NO_REAL_OLLAMA_INTEGRATION_TESTS.md)
