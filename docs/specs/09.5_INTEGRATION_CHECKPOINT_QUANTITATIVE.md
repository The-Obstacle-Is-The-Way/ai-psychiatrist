...
python -m ai_psychiatrist.cli assess --participant-id 300 --mode both
```
...
| N examples | 3 | Exact | Required |
| Chunk size | 5 | Exact | Required |

**Verification**:
```bash
# Run quantitative analysis on dev set
python -m ai_psychiatrist.cli evaluate --split dev --mode quantitative

# Check MAE
python -c "from ai_psychiatrist.evaluation import compute_mae; print(compute_mae('output/dev_quantitative.json'))"
```
...
- [ ] k=3 nearest neighbors retrieval working
...
python -c "from ai_psychiatrist.services import EmbeddingService; es = EmbeddingService(); print(f'Stored: {es.count()}')"
```
...
```bash
# Run on test set
pytest tests/integration/test_quantitative_scoring.py -v

# Manual spot check
python -m ai_psychiatrist.cli assess --participant-id 300 --mode quantitative --verbose
```
...
python -c "
import numpy as np
emb = np.load('data/embeddings/reference_embeddings.npy')
...
python -m ai_psychiatrist.cli debug-retrieval --participant-id 300

# Should show:
# - 3 distinct neighbors
# - Similarity scores
...
| `n_examples` | 3 | Check config.py | |
| `chunk_size` | 5 | Check config.py | |
...
- [ ] k-neighbors matches paper (3)
- [ ] Chunk size matches paper (5)
...
# Coverage for specs 08-09
pytest --cov=src/ai_psychiatrist/services --cov-report=term-missing

# Embedding health check
python -m ai_psychiatrist.cli debug-embeddings

# Quantitative evaluation
python -m ai_psychiatrist.cli evaluate --split dev --mode quantitative

# Anti-pattern detection
grep -rE "4096|k\s*=\s*3" src/ai_psychiatrist/
```
